...
function ResetPassword() {
    ...
    try {
      ...
      if (res && !res.error && (res.message?.includes("OTP sent") || res.message?.includes("generated"))) {
        if (res.testOtp) {
          ...
          setInfoMsg(`OTP sent successfully to ${email}. Please check your inbox!`);
        }
        setStep(2);
      } else {
  ...
  return (
    <div className="login-page-pro">
      <div className="login-card-pro" style={{ maxWidth: 400 }}>
        ...
        <p className="rp-sub-pro">
          {step === 1 && "Enter your registered email address to receive an OTP directly in your inbox."}
                ...
                onClick={async () => {
                  try {
                    ...
                    if (res?.testOtp) {
                      ...
                      alert("New OTP sent to your email inbox!");
                    }
                  } catch (err) {
