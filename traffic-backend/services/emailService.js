...
/**
 * Sends temporary password email to a Traffic Officer after OIC password reset approval.
 * @param {string} toEmail - Registered email address of the officer
 * @param {string} officerName - Full name of the officer
 * @param {string} tempPassword - Plain temporary password generated upon OIC approval
 * @returns {Promise<{success: boolean, error?: string}>}
 */
const sendPasswordResetEmail = async (toEmail, officerName, tempPassword) => {
  const user = process.env.EMAIL_USER;
  const pass = process.env.EMAIL_PASS ? process.env.EMAIL_PASS.replace(/\s+/g, "") : "";
  ...
  if (!toEmail || !toEmail.trim()) {
    console.log("[SMTP] No destination email provided for password reset email.");
    return { success: false, error: "Officer email address is missing." };
  }
  ...
  try {
    ...
    const mailOptions = {
      from: `"Traffic Branch System" <${user}>`,
      to: toEmail.trim(),
      subject: "Traffic Branch System - Password Reset",
      text: `Dear ${officerName},\n\nYour password reset request has been approved by the OIC.\n\nYour temporary password is:\n\n${tempPassword}\n\nPlease use this password to log in to the Traffic Branch System.\n\nFor security, please change your password after logging in.\n\nRegards,\nTraffic Branch Management System`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 24px; background-color: #f8fafc; color: #0f172a; border-radius: 10px; max-width: 560px; margin: 0 auto; border: 1px solid #cbd5e1;">
          <div style="background-color: #0f172a; padding: 18px 24px; border-radius: 8px 8px 0 0; text-align: center;">
            <h2 style="color: #ffffff; margin: 0; font-size: 18px; font-weight: 700; letter-spacing: 0.5px;">SRI LANKA POLICE — TRAFFIC BRANCH</h2>
            <p style="color: #94a3b8; font-size: 12px; margin: 4px 0 0;">Password Reset Approved</p>
          </div>
          <div style="padding: 24px 20px; background-color: #ffffff; border-radius: 0 0 8px 8px;">
            <p style="font-size: 15px; margin-top: 0; color: #1e293b;">Dear <strong>${officerName}</strong>,</p>
            <p style="font-size: 14px; line-height: 1.6; color: #334155;">
              Your password reset request has been approved by the Officer in Charge (OIC).
            </p>

            <div style="background-color: #f8fafc; border-left: 4px solid #0f172a; padding: 16px 20px; margin: 20px 0; border-radius: 4px; border: 1px solid #e2e8f0;">
              <p style="margin: 0 0 8px 0; font-weight: 700; font-size: 14px; color: #0f172a;">Your Temporary Password:</p>
              <p style="margin: 4px 0; font-size: 18px; font-family: monospace; font-weight: 700; color: #1d4ed8; letter-spacing: 1px;">${tempPassword}</p>
            </div>

            <p style="font-size: 14px; line-height: 1.6; color: #334155;">
              Please use this password to log in to the Traffic Branch System.
            </p>
            <p style="font-size: 13px; color: #dc2626; margin-bottom: 24px; font-weight: 600;">
            ...
            <p style="font-size: 12px; color: #94a3b8; margin: 0; line-height: 1.4;">
              Regards,<br />
              <strong>Traffic Branch Management System</strong>
            </p>
          </div>
        </div>
      `
    ...
    await transporter.sendMail(mailOptions);
    console.log(`[SMTP SUCCESS] Password reset email delivered to ${toEmail.trim()} for officer ${officerName}`);
    return { success: true };
  } catch (err) {
    console.error("[SMTP ERROR] Failed to send password reset email:", err.message);
    return { success: false, error: err.message };
  }
};

module.exports = { sendApprovalCredentialsEmail, sendPasswordResetEmail };
